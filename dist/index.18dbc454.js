...
var dawaAutocomplete2 = require("8facc98c36f8ce73");
...
},{"bootstrap":"h36JB","8facc98c36f8ce73":"iXU2D"}],"h36JB":[function(require,module,exports) {
/*!
...
},{"./createPopper.js":"cHuNp","./modifiers/eventListeners.js":"hBKsL","./modifiers/popperOffsets.js":"6I679","./modifiers/computeStyles.js":"gDlm2","./modifiers/applyStyles.js":"4iMn4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"iXU2D":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "dawaAutocomplete", ()=>dawaAutocomplete);
var global = arguments[3];
...
var _wks = createCommonjsModule(function(module) {
    ...
    var Symbol1 = _global.Symbol;
    var USE_SYMBOL = typeof Symbol1 == "function";
    var $exports = module.exports = function(name) {
        return store[name] || (store[name] = USE_SYMBOL && Symbol1[name] || (USE_SYMBOL ? Symbol1 : _uid)("Symbol." + name));
    };
    $exports.store = store;
});
...
if (PATCH) patchedExec = function exec(str) {
    ...
    if (NPCG_INCLUDED) reCopy = new RegExp("^" + re.source + "$(?!\\s)", _flags.call(re));
...
_fixReWks("split", 2, function(defined, SPLIT, $split, maybeCallNative) {
    ...
    return [
        ...
        function(regexp, limit) {
            ...
            if (lim === 0) return [];
            if (S.length === 0) return _regexpExecAbstract(splitter, S) === null ? [
                S
            ] : [];
            var p = 0;
            var q = 0;
            ...
            while(q < S.length){
                ...
                var e;
                if (z === null || (e = $min(_toLength(splitter.lastIndex + (SUPPORTS_Y ? 0 : q)), S.length)) === p) q = _advanceStringIndex(S, q, unicodeMatching);
                else {
                    A.push(S.slice(p, q));
...
var _arrayMethods = function(TYPE, $create) {
    ...
    return function($this, callbackfn, that) {
        ...
        var self1 = _iobject(O);
        var f = _ctx(callbackfn, that, 3);
        var length = _toLength(self1.length);
        var index = 0;
 ...
 */ function assertInPatch(functionName) {
        if (!inPatch) throw new Error("Cannot call " + functionName + "() unless in patch.");
    }
    /**
 ...
 */ function assertPatchOuterHasParentNode(parent) {
        if (!parent) console.warn("patchOuter requires the node have a parent if there is a key.");
    }
    /**
 * Makes sure that the caller is not where attributes are expected.
 * @param functionName The name of the caller, for the error message.
 */ function assertNotInAttributes(functionName) {
        if (inAttributes) throw new Error(functionName + "() can not be called between " + "elementOpenStart() and elementOpenEnd().");
    }
    /**
 * Makes sure that the caller is not inside an element that has declared skip.
 * @param functionName The name of the caller, for the error message.
 */ function assertNotInSkip(functionName) {
        if (inSkip) throw new Error(functionName + "() may not be called inside an element " + "that has called skip().");
    }
    /**
 * Makes sure that the caller is where attributes are expected.
 * @param functionName The name of the caller, for the error message.
 */ function assertInAttributes(functionName) {
        if (!inAttributes) throw new Error(functionName + "() can only be called after calling " + "elementOpenStart().");
    }
    /**
 * Makes sure the patch closes virtual attributes call
 */ function assertVirtualAttributesClosed() {
        if (inAttributes) throw new Error("elementOpenEnd() must be called after calling elementOpenStart().");
    }
    /**
 ...
 */ function assertCloseMatchesOpenTag(currentNameOrCtor, nameOrCtor) {
        if (currentNameOrCtor !== nameOrCtor) throw new Error('Received a call to close "' + nameOrCtor + '" but "' + currentNameOrCtor + '" was open.');
    }
    /**
 ...
 */ function assertNoChildrenDeclaredYet(functionName, previousNode) {
        if (previousNode !== null) throw new Error(functionName + "() must come before any child " + "declarations inside the current element.");
    }
    /**
 ...
 */ function assertPatchElementNoExtras(maybeStartNode, maybeCurrentNode, expectedNextNode, expectedPrevNode) {
        ...
        const wasUpdated = currentNode.nextSibling === expectedNextNode && currentNode.previousSibling === expectedPrevNode;
        const wasChanged = currentNode.nextSibling === startNode.nextSibling && currentNode.previousSibling === expectedPrevNode;
        const wasRemoved = currentNode === startNode;
        if (!wasUpdated && !wasChanged && !wasRemoved) throw new Error("There must be exactly one top level call corresponding to the patched element.");
    }
    /**
 ...
 */ function getNamespace(name) {
        if (name.lastIndexOf("xml:", 0) === 0) return "http://www.w3.org/XML/1998/namespace";
        if (name.lastIndexOf("xlink:", 0) === 0) return "http://www.w3.org/1999/xlink";
        return null;
    }
    /**
 ...
 */ function applyAttr(el, name, value) {
        if (value == null) el.removeAttribute(name);
            ...
            if (attrNS) el.setAttributeNS(attrNS, name, String(value));
 ...
 */ function setStyleValue(style, prop, value) {
        if (prop.indexOf("-") >= 0) style.setProperty(prop, value);
 ...
 */ function applyStyle(el, name, style) {
        ...
        if (typeof style === "string") elStyle.cssText = style;
        else {
            elStyle.cssText = "";
            for(const prop in style)if (has(style, prop)) setStyleValue(elStyle, prop, style[prop]);
        }
    }
    /**
 ...
 */ function applyAttributeTyped(el, name, value) {
        ...
        if (type === "object" || type === "function") applyProp(el, name, value);
        else applyAttr(el, name, value);
    }
    /**
 ...
 */ class Context {
     ...
     */ notifyChanges() {
            if (notifications.nodesCreated && this.created.length > 0) notifications.nodesCreated(this.created);
            if (notifications.nodesDeleted && this.deleted.length > 0) notifications.nodesDeleted(this.deleted);
        }
    }
    /**
 * Copyright 2018 The Incremental DOM Authors. All Rights Reserved.
 ...
 */ function importSingleNode(node, fallbackKey) {
        if (node["__incrementalDOMData"]) return node["__incrementalDOMData"];
        ...
        if (isElement(node)) recordAttributes(node, data);
        return data;
    }
    /**
 ...
 */ function importNode(node) {
        ...
        for(let child = node.firstChild; child; child = child.nextSibling)importNode(child);
    }
    /**
 ...
 */ function clearCache(node) {
        ...
        for(let child = node.firstChild; child; child = child.nextSibling)clearCache(child);
    }
    /**
 * Copyright 2018 The Incremental DOM Authors. All Rights Reserved.
 *
 ...
 */ function getNamespaceForTag(tag, parent) {
        ...
        if (getData(parent).nameOrCtor === "foreignObject") return null;
 ...
 */ function createElement(doc, parent, nameOrCtor, key) {
        let el;
        if (typeof nameOrCtor === "function") el = new nameOrCtor();
        else {
            const namespace = getNamespaceForTag(nameOrCtor, parent);
            if (namespace) el = doc.createElementNS(namespace, nameOrCtor);
            else el = doc.createElement(nameOrCtor);
        }
 ...
 */ function getMatchingNode(matchNode, nameOrCtor, key) {
        ...
        let cur = matchNode;
        do {
            if (matches(cur, nameOrCtor, key)) return cur;
 ...
 */ function getNextNode() {
        ...
        else return currentParent.firstChild;
    }
    /**
 ...
 */ function createNode(nameOrCtor, key) {
        let node;
        if (nameOrCtor === "#text") node = createText(doc);
        else node = createElement(doc, currentParent, nameOrCtor, key);
 ...
 */ function alignWithDOM(nameOrCtor, key) {
        ...
        // from the DOM.
        if (focusPath.indexOf(node) >= 0) // Move everything else before the node.
        moveBefore(currentParent, node, currentNode);
        else currentParent.insertBefore(node, currentNode);
        currentNode = node;
    }
    /**
 ...
 */ function createPatchOuter(patchConfig) {
        return createPatcher((node, fn, data)=>{
            ...
            if (currentParent) clearUnvisitedDOM(currentParent, getNextNode(), node.nextSibling);
 ...
 */ function calculateDiff(prev, next, updateCtx, updateFn) {
        ...
        for(; i < next.length; i += 2){
            const name = next[i];
            if (isNew) prev[i] = name;
            else if (prev[i] !== name) break;
        ...
        if (i < next.length || i < prev.length) {
            const startIndex = i;
            for(i = startIndex; i < prev.length; i += 2)prevValuesMap[prev[i]] = prev[i + 1];
            for(i = startIndex; i < next.length; i += 2){
                const name = next[i];
                const value = next[i + 1];
                if (prevValuesMap[name] !== value) queueChange(updateFn, updateCtx, name, value);
                prev[i] = name;
                prev[i + 1] = value;
 ...
 */ function diffStatics(node, data, statics) {
        if (data.staticsApplied) return;
        data.staticsApplied = true;
        if (!statics || !statics.length) return;
        if (data.hasEmptyAttrsArr()) {
            for(let i = 0; i < statics.length; i += 2)updateAttribute(node, statics[i], statics[i + 1]);
            return;
        }
        for(let i = 0; i < statics.length; i += 2)prevAttrsMap[statics[i]] = i + 1;
        ...
        for(let i = 0; i < attrsArr.length; i += 2){
            ...
            if (staticsIndex) {
                ...
                // not set them again.
                if (statics[staticsIndex] === value) delete prevAttrsMap[name];
                continue;
            }
 ...
 */ function elementOpen(nameOrCtor, key, // Ideally we could tag statics and varArgs as an array where every odd
    ...
    statics, ...varArgs) {
        ...
        for(let i = ATTRIBUTES_OFFSET; i < arguments.length; i += 2)attr(arguments[i], arguments[i + 1]);
...
var renderIncrementalDOM = function renderIncrementalDOM(data, onSelect, multiline) {
    if (data.suggestions.length > 0 && data.focused) {
        ...
        var _loop = function _loop(i) {
            ...
            if (data.selected === i) className += " dawa-selected";
            incrementalDomCjs_27("li", "", [
                "role",
                "option"
            ], "class", className, "onmousedown", function(e) {
                onSelect(i);
                e.preventDefault();
            });
            ...
            } else incrementalDomCjs_32(rows.join(", "));
            incrementalDomCjs_26("li");
        };
        for(var i = 0; i < data.suggestions.length; ++i)_loop(i);
...
var autocompleteUi = function autocompleteUi(inputElm, options) {
    ...
    var keydownHandler = function keydownHandler(e) {
        ...
        if (data.suggestions.length > 0 && data.focused) {
            ...
            if (key === 40) {
                ...
                update();
            } else if (key === 13 || key === 9) selectSuggestion(data.selected);
            else return true;
            e.preventDefault();
            return false;
        }
    ...
    update = function update(shouldUpdateInput) {
        if (shouldUpdateInput) updateInput = true;
        if (!updateScheduled) {
...
if (test + "" != "[object z]") _redefine(Object.prototype, "toString", function toString() {
...
if (!setTask || !clearTask) {
    ...
    // Node.js 0.8-
    if (_cof(process) == "process") defer = function(id) {
    ...
    // IE8-
    } else if (ONREADYSTATECHANGE in _domCreate("script")) defer = function(id) {
    ...
    task.call(_global, function() {
        ...
        if (unhandled) {
            result = _perform(function() {
                ...
                else if (handler = _global.onunhandledrejection) handler({
                    promise: promise,
                    reason: value
                });
                else if ((console1 = _global.console) && console1.error) console1.error("Unhandled promise rejection", value);
            });
...
var onHandleUnhandled = function(promise) {
    task.call(_global, function() {
        ...
        else if (handler = _global.onrejectionhandled) handler({
            promise: promise,
            reason: promise._v
        });
    });
...
var defaultOptions = {
    ...
    renderCallback: function renderCallback() {
        /*eslint no-console: 0*/ console.error("No renderCallback supplied");
    },
    initialRenderCallback: function initialRenderCallback() {
        /*eslint no-console: 0*/ console.error("No initialRenderCallback supplied");
    },
    type: "adresse",
    ...
    _createClass(AutocompleteController, [
        {
            ...
            value: function _getAutocompleteResponse(text, caretpos, skipVejnavn, adgangsadresseid, supplerendebynavn, stormodtagerpostnumre) {
                ...
                if (this.options.fuzzy) params.fuzzy = "";
        ...
        },
        {
            ...
            value: function _scheduleRequest(request) {
                if (this.state.currentRequest !== null) this.state.pendingRequest = request;
                else {
        ...
        },
        {
            ...
            value: function _executeRequest() {
                ...
                } else if (request.selected || request.text.length >= this.options.minLength) this._getAutocompleteResponse(text, caretpos, skipVejnavn, adgangsadresseid, this.options.supplerendebynavn, this.options.stormodtagerpostnumre).then(function(result) {
                    return _this._handleResponse(request, result);
                }, function(error) {
                    return _this._handleFailedRequest(request, error);
                });
                else this._handleResponse(request, []);
            }
        },
        {
            ...
            value: function _handleFailedRequest(request, error) {
                ...
                return delay(this.options.retryDelay).then(function() {
                    if (!_this2.state.pendingRequest) _this2._scheduleRequest(request);
        ...
        },
        {
            ...
            value: function _handleResponse(request, result) {
                if (request.selected) {
                    if (result.length === 1) {
                        var item = result[0];
                        if (item.type === this.options.type) this.options.selectCallback(item);
                        else if (!this.state.pendingRequest) this.state.pendingRequest = {
                            selected: item
                        };
                    } else if (this.options.renderCallback) this.options.renderCallback(result);
        ...
        },
        {
            ...
            value: function _requestCompleted() {
                ...
                if (this.state.currentRequest) this._executeRequest();
            }
        },
        {
...
function dawaAutocomplete(inputElm, options) {
    ...
    var controllerOptions = [
        ...
        "supplerendebynavn",
        "type"
    ].reduce(function(memo, optionName) {
        if (options.hasOwnProperty(optionName)) memo[optionName] = options[optionName];
        return memo;
    }, {});
    if (!controllerOptions.type) {
        if (options.adgangsadresserOnly) controllerOptions.type = "adgangsadresse";
        else controllerOptions.type = "adresse";
    }
    ...
    if (options.id) controller.selectInitial(options.id);
    return {
        id: function id(_id) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["46McK","1SICI"], "1SICI", "parcelRequire6dba")
